...
export default function PaymentTab({ expenses, paymentMethods, onDeleteExpense }) {
    ...
    return (
      <section className={`${cardCls} p-4`}>
        <div className="text-center py-16">
          ...
          <p className={`text-sm ${lm ? 'text-slate-400' : 'text-gray-600'}`}>아직 지출 내역이 없습니다</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <section className={`${cardCls} p-4 flex items-center justify-between`}>
        <div className="flex items-center gap-2">
          <CreditCard className={`w-4 h-4 ${lm ? 'text-indigo-600' : 'text-violet-400'}`} />
          <span className={`text-sm font-bold ${lm ? 'text-slate-700' : 'text-gray-300'}`}>결제 수단별 지출</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            <p className="text-base font-black text-rose-500">₩{fmt(grandTotal)}</p>
            <p className={`text-[10px] ${lm ? 'text-slate-400' : 'text-gray-600'}`}>{groups.length}개 수단</p>
          </div>
        </div>
      </section>
      ...
      {groups.map(({ method, monthGroups, total, count }) => {
        ...
        return (
          <section key={method} className={`${cardCls} overflow-hidden`}>
            <button
              onClick={() => toggleGroup(method)}
              className={`w-full px-4 py-3 border-b flex items-center justify-between ${lm ? 'border-slate-100 hover:bg-slate-50' : 'border-gray-800'}`}
            >
              <div className="flex items-center gap-2">
                <span className={`text-sm font-bold ${lm ? 'text-slate-800' : 'text-gray-200'}`}>{method}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${lm ? 'text-slate-400 bg-slate-100' : 'text-gray-600 bg-gray-800'}`}>
                  {count}건
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-sm font-black ${lm ? 'text-indigo-600' : 'text-violet-400'}`}>₩{fmt(total)}</span>
                {isCollapsed
            ...
            {!isCollapsed && (
              <div>
                {monthGroups.map(([monthKey, items]) => (
                  <div key={monthKey}>
                    <div className={`flex items-center justify-between px-4 py-2 ${lm ? 'bg-slate-50' : 'bg-gray-800/50'}`}>
                    ...
                    <ul className={`divide-y ${lm ? 'divide-slate-100' : 'divide-gray-800/60'}`}>
                      {items.map(e => (
                        <li key={e.id} className="flex items-center justify-between px-4 py-3">
                          <div className="min-w-0 flex-1">
                            <p className={`text-sm font-medium truncate ${lm ? 'text-slate-800' : 'text-gray-100'}`}>{e.name}</p>
                            <p className={`text-[10px] mt-0.5 ${lm ? 'text-slate-400' : 'text-gray-600'}`}>{e.date}</p>
                            {e.memo && (
