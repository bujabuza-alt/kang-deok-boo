...
function RandomPicker({ menus, lm }) {
  ...
  return (
    <button
      ...
      className={`w-full rounded-2xl border-2 p-5 flex flex-col items-center gap-2 transition-all duration-200 select-none ${
        spinning
          ? lm ? 'border-indigo-300 bg-indigo-50 cursor-wait' : 'border-violet-700 bg-violet-950/30 cursor-wait'
          : picked
          ? lm ? 'border-indigo-500 bg-indigo-600 hover:bg-indigo-700 active:scale-[0.98]' : 'border-violet-500 bg-violet-600 hover:bg-violet-500 active:scale-[0.98]'
          : lm
            ? 'border-dashed border-slate-300 hover:border-indigo-300 hover:bg-indigo-50 active:scale-[0.98]'
            : 'border-dashed border-gray-700 hover:border-violet-600 hover:bg-violet-950/20 active:scale-[0.98]'
      }`}
    >
      <div className="flex items-center gap-2">
        <Shuffle
          className={`w-5 h-5 transition-colors ${
            spinning ? (lm ? 'animate-spin text-indigo-500' : 'animate-spin text-violet-400') : picked ? 'text-white' : lm ? 'text-slate-400' : 'text-gray-500'
          }`}
        />
        <span
          className={`font-bold text-base transition-colors ${
            spinning ? (lm ? 'text-indigo-600' : 'text-violet-400') : picked ? 'text-white' : lm ? 'text-slate-500' : 'text-gray-400'
          }`}
        >
          뭐 먹지?
        </span>
      ...
      {spinning && (
        <span className={`text-sm animate-pulse ${lm ? 'text-indigo-500' : 'text-violet-400'}`}>고르는 중...</span>
      )}
      {picked && !spinning && (
        <span className="text-xl font-bold text-white tracking-tight">{picked}</span>
      )}
      {!picked && !spinning && (
        <span className={`text-xs ${lm ? 'text-slate-400' : 'text-gray-500'}`}>누르면 랜덤으로 골라줘요</span>
      )}
    </button>
  );
}
...
export function MealMenu({ triggerAdd = false, onTriggerAddDone }) {
  ...
  return (
    <div className="max-w-3xl mx-auto w-full px-4 py-6 flex flex-col gap-4">
      <RandomPicker menus={allMenus} lm={lm} />
      ...
      <div className="flex gap-2">
        <div className={`flex-1 flex items-center gap-2 px-3 py-2 rounded-xl ${lm ? 'bg-slate-100' : 'bg-gray-800'}`}>
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className={lm ? 'text-slate-400 hover:text-slate-600' : 'text-gray-500 hover:text-gray-300'}>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-sm font-medium transition-colors ${
            lm ? 'border-slate-200 text-slate-600 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600' : 'border-gray-700 text-gray-300 hover:bg-violet-950/30 hover:border-violet-700 hover:text-violet-400'
          }`}
        >
      ...
      {adding && (
        <form onSubmit={handleAdd} className={`rounded-2xl border p-4 shadow-sm flex gap-2 ${lm ? 'bg-white border-slate-200' : 'bg-gray-900 border-gray-800'}`}>
          <input
            autoFocus
            value={newName}
            onChange={(e) => setNewName(e.target.value)}
            placeholder="새 메뉴 이름"
            className={`flex-1 px-3 py-2 rounded-xl border text-sm focus:outline-none ${lm ? 'border-slate-200 focus:border-indigo-300 text-slate-800' : 'border-gray-700 focus:border-violet-600 bg-gray-800 text-white'}`}
          />
          <button
            type="button"
            onClick={() => { setAdding(false); setNewName(''); }}
            className={`p-2 rounded-xl border transition-colors ${lm ? 'border-slate-200 text-slate-500 hover:bg-slate-50' : 'border-gray-700 text-gray-400 hover:bg-gray-800'}`}
          >
            <X className="w-4 h-4" />
          </button>
          <button
      ...
      <div className={`text-xs px-0.5 ${lm ? 'text-slate-400' : 'text-gray-500'}`}>
      ...
      {filtered.length === 0 ? (
        <div className={`flex flex-col items-center justify-center py-16 ${lm ? 'text-slate-300' : 'text-gray-700'}`}>
          <Search className="w-10 h-10 mb-3" />
          <p className={`text-sm ${lm ? 'text-slate-400' : 'text-gray-500'}`}>검색 결과가 없어요</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                  isCustom
                    ? lm ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-violet-950/30 text-violet-300 border-violet-800'
                    : lm ? 'bg-white text-slate-700 border-slate-200' : 'bg-gray-900 text-gray-300 border-gray-800'
                }`}
              >
                <span>{name}</span>
                {isCustom && (
                  <button
                    onClick={() => remove(name)}
                    className={`ml-0.5 transition-colors ${lm ? 'text-indigo-300 hover:text-indigo-600' : 'text-violet-500 hover:text-violet-300'}`}
                  >
                    <X className="w-3 h-3" />
                  </button>
                )}
